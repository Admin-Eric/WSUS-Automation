...
$group = $updateServer.GetComputerTargetGroups | where {$_Name -eq "Update Testing"}
...
function Approve-Nonsuperseded {
    ...
    foreach ($u2 in $u)
        {
        if (($u2.IsSuperseded -eq 'False') -and ($u2.ArrivalDate -ge $patchday) -and ($u2.IsDeclined -eq 'False'))
            {
                write-host Approving Update : $u2.Title
                $u2.Approve("Install","Update Testing")
                $count = $count + 1
            }
        }
